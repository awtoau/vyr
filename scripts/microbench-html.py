...
it after every measurement; run standalone to re-render without measuring:
    ...
    python3 scripts/microbench-html.py [--db docs/perf/ledger.db (mb_run / mb_points tables)] [--run N]
...
def load(db_path: Path, run: int | None) -> tuple[dict, list[dict]]:
    ...
    if run is None:
        run = con.execute("SELECT max(run_id) FROM mb_run").fetchone()[0]
    meta = dict(con.execute("SELECT * FROM mb_run WHERE run_id=?", (run,)).fetchone())
    rows = [dict(r) for r in con.execute(
        "SELECT * FROM mb_points WHERE run_id=? AND name!='null' ORDER BY tier,name", (run,))]
    con.close()
    return meta, rows
...
generated by scripts/microbench-html.py from docs/perf/ledger.db (mb_run / mb_points tables)</div>
