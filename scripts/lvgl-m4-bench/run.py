...
def gen_checker_header(logf):
    """Emit checker-24.inc from vyr's OWN committed asset.

    #27 Task B: the harness used to build its own two-grey 6x6 checkerboard,
    so the image widget was comparing a grey mock-up against vyr's coloured
    24x24 test asset — a content difference, not a renderer difference, right
    in the middle of a published side-by-side. Both sides now blit the SAME
    bytes: vyr-size/assets/checker-24.rgba, straight RGBA, converted here to
    LVGL's ARGB8888 memory order (B,G,R,A little-endian). Regenerated on every
    build so it cannot go stale against the asset.
    """
    raw = open(os.path.join(REPO, "vyr-size", "assets", "checker-24.rgba"), "rb").read()
    w = h = 24
    if len(raw) != w * h * 4:
        raise SystemExit(f"checker-24.rgba is {len(raw)} B, expected {w * h * 4}")
    out = [
        "/* GENERATED by scripts/lvgl-m4-bench/run.py — do not edit. */",
        "/* Source: vyr-size/assets/checker-24.rgba (straight RGBA), the SAME",
        "   asset vyr's FIXTURE_IR blits. Converted to LVGL ARGB8888 memory",
        "   order: B, G, R, A. */",
        f"#define CK_W {w}",
        f"#define CK_H {h}",
        "static const uint8_t g_checker_px[CK_W * CK_H * 4] = {",
    ]
    row = []
    for i in range(0, len(raw), 4):
        r, g, b, a = raw[i], raw[i + 1], raw[i + 2], raw[i + 3]
        row.append(f"{b},{g},{r},{a},")
        if len(row) == 6:
            out.append("    " + "".join(row))
            row = []
    if row:
        out.append("    " + "".join(row))
    out.append("};")
    path = os.path.join(HERE, "checker-24.inc")
    open(path, "w").write("\n".join(out) + "\n")
    log(f"generated {path} from vyr-size/assets/checker-24.rgba "
        f"({len(raw)} B, ARGB8888 order)", logf)
    return path
